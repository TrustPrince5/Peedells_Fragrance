...
function Login () {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[url('/img/Onyx.jpg')] bg-cover bg-center relative">
       <div className="absolute inset-0 bg-black/50"></div>
      <div className="relative flex flex-col md:flex-row items-center justify-between h-full px-6 md:px-12 space-y-10 md:space-y-20 md:space-x-60 text-gray-300 font-poppins">
      ...
      <div className="w-full max-w-md backdrop-blur-md rounded-2xl shadow-lg p-4 space-y-2 font-poppins my-12">
        <p className="text-md font-bold text-[#D8F1A0] md:text-2xl my-6 text-center md:text-left">Please enter your credentials</p>
        <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="relative">
            ...
            <input id="login-password" type={showPassword ? "text" : "password"}
              {...register("password", { required: "User Password is required" })}
                placeholder='**********' className="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-transparent"/>

            <button type="button" className="absolute right-3 bottom-2 text-sm font-medium text-[#D8F1A0] hover:text-[#a3ad8c] focus:outline-none"
          ...
          <button type="submit" disabled={isSubmitting}
            className=" items-center w-50 py-3 border border-transparent rounded-md shadow-sm bg-[#0B0500] duration-500 hover:bg-[#D8F1A0] hover:text-[#0B0500] text-[#D8F1A0] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
            {isSubmitting ? (
        ...
        <div className="text-center text-sm text-gray-200 my-8">
